...
const PUSHTIMEOUT = functions.setEnv(process.env.PUSH_TIMEOUT, 15000);
const AHK_INVITER = functions.setEnv(process.env.AHK_INVITER.toLowerCase(), "false");
...
module.exports = {
    ...
    pushdelete: function(message) {
        ...
        for (p in pushes) {
            if (p == message.channel.id) {
                ...
                delete pushes[p];
                functions.reply(message, "Push deleted.");
    ...
    queue: function(message) {
        module.exports.queuejoin(message, 6);
    },
    ...
    leave: function(message) {
        module.exports.queueleave(message, 6);
    },
    ...
    createnewmessage: function(message) {
        ...
        if (!message.member.roles.find(role => role.name === pushes[message.channel.id]["leaders"])) {
            pushReply(message, "You require the " + pushes[message.channel.id]["leaders"] + " role to create a new push message.");
            return;
        }
        ...
        //send embed
        functions.reply(message, "Processing...")
        .then(m => {
